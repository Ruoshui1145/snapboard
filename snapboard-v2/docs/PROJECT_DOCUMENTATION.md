...
长圆孔锚点可保存局部安装面内单位向量 `axis`；装配求解要求旋转后的轴与板孔 `[0,1]` 满足 `|dot| ≥ 0.9`。打开标定器后会通过端面环形采样补算；同一零件上方向一致的已标定锚点还会在资源同步时补齐采样失败的锚点，避免“明明标过方向”仍被整体标成待补。真正互相冲突或全部缺失的方向仍需人工复核。所有已放置零件的 `targetIds` 汇总成正背面共享的占用集合；自动装配命中未开候选圆孔时执行幂等开孔。
